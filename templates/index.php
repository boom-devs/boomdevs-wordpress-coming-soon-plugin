<?php 
	global $cst_taspristudio;
?>
<!DOCTYPE html>
<html lang="en">
<head>
	<title><?php _e( 'Coming soon', 'cs_taspristudio' ); ?></title>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<?php 
		if ( ! empty( $cst_taspristudio['cst_fevicon'] ['url'] ) ) { ?>
			<link rel="shortcut icon" type="fevicon" href="<?php echo esc_url( $cst_taspristudio['cst_fevicon'] ['url'] ); ?>">
		<?php } else { ?>
			<link rel="shortcut icon" type="fevicon" href="<?php echo CST_ASSETS; ?>/images/fevicon.png">
	<?php } ?>

	<link rel="stylesheet" href="<?php echo CST_ASSETS; ?>/css/font-awesome.min.css">
	<link rel="stylesheet" href="<?php echo CST_ASSETS; ?>/css/bootstrap.min.css">
	<link rel="stylesheet" href="<?php echo CST_ASSETS; ?>/css/style.css">
	<link rel="stylesheet" href="<?php echo CST_ASSETS; ?>/css/responsive.css">
</head>
<body data-spy="click" data-target=".navbar-nav">
	<div class="page-wrapper">
		<header class="header">
			<nav class="navbar navbar-expand-lg fixed-top">
				<div class="container">
					<a class="navbar-brand" href="#home">
						<?php 
							if ( ! empty( $cst_taspristudio['cst_display_logo'] ) ) { 
								if ( ! empty( $cst_taspristudio['cst_header_logo'] ['url'] ) ) { ?>
									<img src="<?php echo esc_url( $cst_taspristudio['cst_header_logo'] ['url'] ); ?>" alt="">
								<?php } else { ?>
									<img src="<?php echo CST_ASSETS; ?>/images/logo.png" alt="">
								<?php } ?>
						<?php } ?>
					</a>
					<button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#main-nav" aria-controls="main-nav" aria-expanded="false" aria-label="Toggle navigation">
						<div class="menu-toggle">
	                        <div class="hamburger">
	                            <span></span>
	                            <span></span>
	                            <span></span>
	                        </div>
	                        <div class="hamburger-cross">
	                            <span></span>
	                            <span></span>
	                        </div>
	                    </div>
					</button>
					<div class="collapse navbar-collapse" id="main-nav">
						<ul class="navbar-nav ml-auto">
							<!-- Home menu -->
							<?php
								$home_menu_name 	= strtolower( $cst_taspristudio['cst_home_menu_name'] );
								$home_menu 			= preg_replace('/[^a-z0-9_\-]/', '', $home_menu_name);
								?>
						 		<li class="nav-item active">
									<a class="nav-link" href="#<?php echo esc_html( $home_menu ); ?>">
										<?php echo esc_html( ucfirst( $home_menu ) ); ?>
									</a>
								</li>
							<!-- Service menu -->
							<?php
								if ( ! empty( $cst_taspristudio['cst_service_status'] ) ) {
									$service_menu_name 	= strtolower( $cst_taspristudio['cst_service_menu_name'] );
									$service_menu 		= preg_replace('/[^a-z0-9_\-]/', '', $service_menu_name);
									?>
							 		<li class="nav-item">
										<a class="nav-link" href="#<?php echo esc_html( $service_menu ); ?>">
											<?php echo esc_html( ucfirst( $service_menu ) ); ?>
										</a>
									</li>
							<?php } ?>
							<!-- Blog menu -->
							<?php
								if ( ! empty( $cst_taspristudio['cst_blog_status'] ) ) {
									$blog_menu_name 	= strtolower( $cst_taspristudio['cst_blog_menu_name'] );
									$blog_menu 			= preg_replace('/[^a-z0-9_\-]/', '', $blog_menu_name);
									?>
							 		<li class="nav-item">
										<a class="nav-link" href="#<?php echo esc_html( $blog_menu ); ?>">
											<?php echo esc_html( ucfirst( $blog_menu ) ); ?>
										</a>
									</li>
							<?php } ?>
							<!-- Contact menu -->
							<?php
								if ( ! empty( $cst_taspristudio['cst_contact_status'] ) ) {
									$contact_menu_name 	= strtolower( $cst_taspristudio['cst_contact_menu_name'] );
									$contact_menu 		= preg_replace('/[^a-z0-9_\-]/', '', $contact_menu_name);
									?>
							 		<li class="nav-item">
										<a class="nav-link" href="#<?php echo esc_html( $contact_menu ); ?>">
											<?php echo esc_html( ucfirst( $contact_menu ) ); ?>
										</a>
									</li>
							<?php } ?>
						</ul>
					</div>
				</div>
			</nav>
		</header>
		<!-- header -->

		<div class="main-content">
			<!-- Home section -->
			<div class="countdown-area toggle-section show" id="<?php echo esc_html( $home_menu ); ?>">
				<div class="container">
					<div class="row align-items-center">
						<div class="col">
							<div class="countdown-timer">
								<div class="countdown" data-countdown="<?php echo esc_html( $cst_taspristudio['cst_end_date'] ); ?> <?php echo esc_html( $cst_taspristudio['cst_time_hour'] ); ?>:<?php echo esc_html( $cst_taspristudio['cst_time_minute'] ); ?>"></div>
							</div>
						</div>
						<div class="col">
							<div class="coming-soon-content">
								<h2><?php echo $cst_taspristudio['cst_home_title']; ?></h2>
								<p><?php echo $cst_taspristudio['cst_home_desc']; ?></p>
								<form class="newsletter">
									<div class="input-group">
										<input type="text" class="form-control" placeholder="Your email Here" aria-label="Your email Here" autocomplete="off">
										<span class="input-group-btn">
											<button class="btn newsletter-btn" type="button">Subscribe Now!</button>
										</span>
								    </div>
								</form>
							</div>
						</div>
					</div>
				</div>
			</div>
			<!-- Service section -->
			<?php if( ! empty( $cst_taspristudio['cst_service_status'] ) ) { ?>
			<div class="services toggle-section" id="<?php echo esc_html( $service_menu ); ?>">
				<div class="container">
					<div class="row">
						<div class="col-12">
							<div class="s-title">
								<h2><?php echo $cst_taspristudio['cst_service_title']; ?></h2>
								<p><?php echo $cst_taspristudio['cst_service_desc']; ?></p>
							</div>
						</div>
						<div class="col-lg-3 col-sm-6">
							<div class="service-item">
								<div class="item-icon">
									<svg xmlns="http://www.w3.org/2000/svg" viewBox="5959 1070 79 79">
									  <g id="flaticon1521640920-svg" transform="translate(5959 1070)">
									    <path id="Shape" class="cls-1" d="M16.714,10H1.286A1.27,1.27,0,0,1,0,8.75,8.888,8.888,0,0,1,9,0a8.888,8.888,0,0,1,9,8.75A1.27,1.27,0,0,1,16.714,10Zm-9-7.362A6.323,6.323,0,0,0,2.7,7.5H15.3a6.325,6.325,0,0,0-5.013-4.862V5H7.714V2.638Z" transform="translate(0 69)"/>
									    <path id="Shape-2" data-name="Shape" class="cls-1" d="M65.833,79h-7.9a1.318,1.318,0,0,1-1.317-1.317V75.374l-.038-.014-.042-.017-.045-.019-.012,0-.009,0c-.089-.036-.254-.1-.4-.171l-1.634,1.634a1.315,1.315,0,0,1-1.861,0l-5.585-5.585a1.316,1.316,0,0,1,0-1.862L48.622,67.7c-.081-.18-.156-.36-.229-.548H46.084a1.318,1.318,0,0,1-1.317-1.317H16.458a4.608,4.608,0,0,1,0-9.217h2.333L23.765,47.4H6.583A6.591,6.591,0,0,1,0,40.817V6.583A6.591,6.591,0,0,1,6.583,0H67.15a6.591,6.591,0,0,1,6.583,6.583V40.817a6.569,6.569,0,0,1-3.529,5.8l.025,0h.042a1.292,1.292,0,0,1,.922.376l5.585,5.585a1.317,1.317,0,0,1,0,1.861l-1.634,1.634c.057.127.11.255.156.367l.021.05.016.04.011.027.025.062h2.309A1.318,1.318,0,0,1,79,57.933v7.9a1.318,1.318,0,0,1-1.317,1.317H75.374c-.1.244-.163.4-.229.548l1.634,1.634a1.316,1.316,0,0,1,0,1.862l-5.585,5.585a1.316,1.316,0,0,1-1.862,0L67.7,75.145c-.167.073-.326.14-.548.229v2.309A1.318,1.318,0,0,1,65.833,79ZM55.8,72.238a1.326,1.326,0,0,1,.608.148,11.888,11.888,0,0,0,1.922.8,1.313,1.313,0,0,1,.92,1.256v1.921h5.267V74.445a1.317,1.317,0,0,1,.922-1.257,11.8,11.8,0,0,0,1.922-.8,1.327,1.327,0,0,1,.608-.149,1.31,1.31,0,0,1,.931.385l1.364,1.364,3.723-3.723L72.624,68.9a1.314,1.314,0,0,1-.238-1.541,11.912,11.912,0,0,0,.8-1.921,1.313,1.313,0,0,1,1.256-.921h1.921V59.25H74.445a1.317,1.317,0,0,1-1.257-.922,11.824,11.824,0,0,0-.8-1.921,1.313,1.313,0,0,1,.237-1.541L73.986,53.5l-3.723-3.723L68.9,51.143a1.316,1.316,0,0,1-1.541.237,11.824,11.824,0,0,0-1.921-.8,1.314,1.314,0,0,1-.921-1.256V47.4H59.25v1.921a1.316,1.316,0,0,1-.922,1.257,11.912,11.912,0,0,0-1.921.8,1.328,1.328,0,0,1-.61.149,1.3,1.3,0,0,1-.931-.387L53.5,49.781,49.779,53.5l1.364,1.364a1.311,1.311,0,0,1,.237,1.54,11.8,11.8,0,0,0-.8,1.922,1.314,1.314,0,0,1-1.256.92H47.4v5.267h1.921a1.316,1.316,0,0,1,1.257.922,11.888,11.888,0,0,0,.8,1.922,1.313,1.313,0,0,1-.238,1.54l-1.363,1.364L53.5,73.987l1.364-1.363A1.31,1.31,0,0,1,55.8,72.238ZM16.458,59.25a1.975,1.975,0,0,0,0,3.95H44.767V59.25H16.458ZM26.757,47.4l-4.973,9.215H48.393c.082-.206.157-.386.229-.548l-1.633-1.633a1.318,1.318,0,0,1,0-1.862L49.13,50.43,47.065,47.4Zm42.557-.4a6.511,6.511,0,0,1-2.164.4v.993c.237.094.407.165.548.229Zm-14.467.4,1.222,1.221c.189-.084.368-.159.547-.229V47.4Zm-4.6,0,.773,1.135L52.16,47.4ZM2.633,36.867v3.95a3.955,3.955,0,0,0,3.95,3.95H67.15a3.955,3.955,0,0,0,3.95-3.95v-3.95ZM6.583,2.633a3.955,3.955,0,0,0-3.95,3.95v27.65H71.1V6.583a3.955,3.955,0,0,0-3.95-3.95Z"/>
									    <path id="Shape-3" data-name="Shape" class="cls-1" d="M8,16a8,8,0,1,1,8-8A8.009,8.009,0,0,1,8,16ZM8,2.667A5.333,5.333,0,1,0,13.333,8,5.339,5.339,0,0,0,8,2.667Z" transform="translate(54 54)"/>
									    <path id="Shape-4" data-name="Shape" class="cls-1" d="M12,1.926,10.051,0,.4,9.537a1.35,1.35,0,0,0,0,1.926L10.051,21,12,19.074,3.327,10.5Z" transform="translate(14 9)"/>
									    <path id="Shape-5" data-name="Shape" class="cls-1" d="M11.6,9.537,1.949,0,0,1.926,8.673,10.5,0,19.074,1.949,21,11.6,11.463A1.35,1.35,0,0,0,11.6,9.537Z" transform="translate(44 9)"/>
									    <rect id="Rectangle-path" class="cls-1" width="27" height="3" transform="translate(28.121 28.904) rotate(-63.435)"/>
									    <rect id="Rectangle-path-2" data-name="Rectangle-path" class="cls-1" width="3" height="3" transform="translate(51 27)"/>
									    <rect id="Rectangle-path-3" data-name="Rectangle-path" class="cls-1" width="3" height="3" transform="translate(56 27)"/>
									    <rect id="Rectangle-path-4" data-name="Rectangle-path" class="cls-1" width="3" height="3" transform="translate(62 27)"/>
									  </g>
									</svg>
								</div>
								<h4 class="item-title">Web Design & Development</h4>
								<p>From Web Design to Web Application Developement we can do whatever you want to fullfull your needs.</p>
							</div>
						</div>
						<div class="col-lg-3 col-sm-6">
							<div class="service-item">
								<div class="item-icon">
									<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="6319 1070 71 69">
									  <g id="flaticon1521559238-svg" transform="translate(6319 1070)">
									    <path id="Shape" class="cls-1" d="M52.141,69H18.859V66.775h2.219V63.436a1.113,1.113,0,0,1,1.11-1.113h2.418l1.775-8.9H1.11A1.113,1.113,0,0,1,0,52.306V3.339A3.337,3.337,0,0,1,3.328,0H67.672A3.337,3.337,0,0,1,71,3.339V52.306a1.113,1.113,0,0,1-1.11,1.113H44.618l1.775,8.9h2.42a1.113,1.113,0,0,1,1.11,1.113v3.339h2.218V69ZM23.3,64.548v2.227H47.7V64.548H23.3ZM28.642,53.42l-1.773,8.9H44.131l-1.775-8.9ZM2.218,41.178V51.194H68.782V41.178H2.218ZM6.656,5.565H64.344a1.112,1.112,0,0,1,1.11,1.112V38.952h3.328V3.339a1.113,1.113,0,0,0-1.11-1.113H3.328a1.113,1.113,0,0,0-1.11,1.113V38.952H5.547V6.677A1.112,1.112,0,0,1,6.656,5.565Zm37.719,4.451H53.25a1.112,1.112,0,0,1,1.109,1.113V38.952h8.875V7.79H7.765V38.952h8.876V17.807a1.058,1.058,0,0,1,.089-.438l3.328-7.79A1.11,1.11,0,0,1,21.079,8.9a1.129,1.129,0,0,1,.437.09,1.1,1.1,0,0,1,.582.584l3.328,7.79a1.146,1.146,0,0,1,.09.438v3.339H23.3V18.92H22.188V38.952H23.3v-8.9h2.219v8.9h4.437V33.387c0-4.557-5.5-5.54-5.732-5.58a1.115,1.115,0,0,1-.907-1.284,1.1,1.1,0,0,1,.142-.385L33.439,9.444a1.151,1.151,0,0,1,1.9,0l9.985,16.694a1.113,1.113,0,0,1-.769,1.67c-.234.04-5.729,1.044-5.729,5.58v5.565h4.438v-8.9h2.218v8.9h6.656V34.5H47.7V32.274h4.438V26.709H47.7V24.484h4.438V18.92H47.7V16.693h4.438V12.242H45.484v8.9H43.266V11.129A1.113,1.113,0,0,1,44.375,10.016Zm-12.2,27.823v1.113h4.437V37.839ZM18.859,18.92V38.952h1.109V18.92ZM32.172,34.5v1.113h4.437V34.5Zm1.109-20.465L26.122,26c1.592.539,5.361,2.232,5.969,6.271h4.6c.608-4.055,4.377-5.737,5.969-6.271L35.5,14.035v9.337H33.281Zm-12.2-1.2L19.43,16.693h3.3Z"/>
									    <path id="Shape-2" data-name="Shape" class="cls-1" d="M3.5,7A3.5,3.5,0,1,1,7,3.5,3.5,3.5,0,0,1,3.5,7Zm0-4.667A1.166,1.166,0,1,0,4.667,3.5,1.168,1.168,0,0,0,3.5,2.334Z" transform="translate(32 42)"/>
									  </g>
									</svg>
								</div>
								<h4 class="item-title">UI UX Design</h4>
								<p>From Web Design to Web Application Developement we can do whatever you want to fullfull your needs.</p>
							</div>
						</div>
						<div class="col-lg-3 col-sm-6">
							<div class="service-item">
								<div class="item-icon">
									<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="6679 1070 82 69">
									  <g id="flaticon1521641627-svg" transform="translate(6679 1070)">
									    <path id="Shape" class="cls-1" d="M22.855,27.789H10.1L2.478,14.1,9.313,1.848a1.27,1.27,0,0,0-.4-1.676A1.105,1.105,0,0,0,7.345.6L.161,13.476a1.293,1.293,0,0,0,0,1.25L7.433,27.789H5.947A1.186,1.186,0,0,0,4.8,29.012v9.766A1.186,1.186,0,0,0,5.947,40H9.709a1.225,1.225,0,0,0,0-2.445H7.092V30.234H9.423l.024,0H22.855A1.186,1.186,0,0,0,24,29.013v0A1.186,1.186,0,0,0,22.855,27.789Z" transform="translate(20 28)"/>
									    <path id="Shape-2" data-name="Shape" class="cls-1" d="M56.678,69H33.95a1.2,1.2,0,1,1,0-2.394H55.475V59.439H47.752a1.153,1.153,0,0,1-.825-.34,1.362,1.362,0,0,1-.371-.98,1.2,1.2,0,0,1,1.2-1.075H52.32l8-13.4-18.185-30.4V35.405a4.81,4.81,0,1,1-2.4,0V13.247L31.2,27.5a1.211,1.211,0,0,1-1.035.585,1.23,1.23,0,0,1-.614-.168,1.193,1.193,0,0,1-.418-1.641L38.911,9.938H37.143a1.2,1.2,0,0,1-1.2-1.2V6.594A34.694,34.694,0,0,0,6.154,39.721H8.783a1.2,1.2,0,0,1,1.2,1.2v7.544a1.2,1.2,0,0,1-1.2,1.2H1.2a1.2,1.2,0,0,1-1.2-1.2V40.918a1.2,1.2,0,0,1,1.2-1.2H3.748a36.843,36.843,0,0,1,9.609-23.705A37.227,37.227,0,0,1,27.239,6.435H9.806A4.994,4.994,0,0,1,0,5.126,4.993,4.993,0,0,1,9.86,4.042h26.08V1.2a1.2,1.2,0,0,1,1.2-1.2h7.581a1.2,1.2,0,0,1,1.2,1.2V4.042h26.08a4.988,4.988,0,1,1,4.87,6.052,5,5,0,0,1-4.814-3.659H54.627A37.227,37.227,0,0,1,68.51,16.016a36.843,36.843,0,0,1,9.609,23.705H80.8a1.2,1.2,0,0,1,1.2,1.2v7.544a1.2,1.2,0,0,1-1.2,1.2H73.217a1.2,1.2,0,0,1-1.2-1.2V40.918a1.2,1.2,0,0,1,1.2-1.2h2.5A34.7,34.7,0,0,0,45.926,6.594V8.741a1.2,1.2,0,0,1-1.2,1.2H42.955l19.8,33.095a1.19,1.19,0,0,1,0,1.223l-7.636,12.79h1.561a1.2,1.2,0,0,1,1.2,1.2V67.8A1.2,1.2,0,0,1,56.678,69ZM74.419,42.114v5.151H79.6V42.114Zm-72.014,0v5.151H7.581V42.114Zm38.529-4.467a2.393,2.393,0,1,0,2.4,2.393A2.4,2.4,0,0,0,40.934,37.647ZM76.876,2.553a2.574,2.574,0,1,0,2.586,2.573A2.584,2.584,0,0,0,76.876,2.553Zm-71.885,0A2.574,2.574,0,1,0,7.577,5.126,2.583,2.583,0,0,0,4.991,2.553Zm33.354-.159v5.15h5.176V2.394Z"/>
									  </g>
									</svg>
								</div>
								<h4 class="item-title">Digital Branding & Identity</h4>
								<p>From Web Design to Web Application Developement we can do whatever you want to fullfull your needs.</p>
							</div>
						</div>
						<div class="col-lg-3 col-sm-6">
							<div class="service-item">
								<div class="item-icon">
									<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="7039 1070 71 69">
									  <g id="flaticon1521642215-svg" transform="translate(7039 1070)">
									    <path id="Shape" class="cls-1" d="M46.2,69H1.184A1.169,1.169,0,0,1,0,67.85V5.75A1.169,1.169,0,0,1,1.184,4.6H4.739V1.15A1.169,1.169,0,0,1,5.924,0H50.942a1.169,1.169,0,0,1,1.184,1.15v62.1a1.169,1.169,0,0,1-1.184,1.15H47.388v3.45A1.169,1.169,0,0,1,46.2,69ZM2.37,6.9V66.7H45.018V6.9ZM7.108,2.3V4.6H46.2a1.169,1.169,0,0,1,1.184,1.15V62.1h2.37V2.3Z"/>
									    <path id="Shape-2" data-name="Shape" class="cls-1" d="M5.779,69a1.157,1.157,0,0,1-.963-.512l-4.621-6.9A1.141,1.141,0,0,1,0,60.95V1.15A1.154,1.154,0,0,1,1.156,0H10.4a1.154,1.154,0,0,1,1.156,1.15V4.715a5.767,5.767,0,0,1,4.622,5.635v11.5H13.866V10.35a3.458,3.458,0,0,0-2.311-3.238V60.95a1.141,1.141,0,0,1-.194.638l-4.621,6.9a1.159,1.159,0,0,1-.96.512ZM2.311,32.2v28.4l3.467,5.174,3.466-5.174V32.2Zm0-29.9V29.9H9.244V2.3Z" transform="translate(54.823)"/>
									    <rect id="Rectangle-path" class="cls-1" width="23.367" height="2.62" transform="translate(11.684 10.481)"/>
									    <rect id="Rectangle-path-2" data-name="Rectangle-path" class="cls-1" width="7.19" height="2.62" transform="translate(11.684 19.215)"/>
									    <rect id="Rectangle-path-3" data-name="Rectangle-path" class="cls-1" width="13.481" height="2.62" transform="translate(21.57 19.215)"/>
									    <rect id="Rectangle-path-4" data-name="Rectangle-path" class="cls-1" width="8.987" height="2.62" transform="translate(7.19 24.456)"/>
									    <rect id="Rectangle-path-5" data-name="Rectangle-path" class="cls-1" width="11.684" height="2.62" transform="translate(18.873 24.456)"/>
									    <rect id="Rectangle-path-6" data-name="Rectangle-path" class="cls-1" width="33.253" height="2.62" transform="translate(7.19 28.823)"/>
									    <rect id="Rectangle-path-7" data-name="Rectangle-path" class="cls-1" width="7.19" height="2.62" transform="translate(7.19 33.19)"/>
									    <rect id="Rectangle-path-8" data-name="Rectangle-path" class="cls-1" width="23.367" height="2.62" transform="translate(16.177 33.19)"/>
									    <rect id="Rectangle-path-9" data-name="Rectangle-path" class="cls-1" width="33.253" height="2.62" transform="translate(7.19 37.557)"/>
									    <rect id="Rectangle-path-10" data-name="Rectangle-path" class="cls-1" width="3.595" height="2.62" transform="translate(36.848 19.215)"/>
									    <rect id="Rectangle-path-11" data-name="Rectangle-path" class="cls-1" width="7.19" height="2.62" transform="translate(33.253 24.456)"/>
									    <rect id="Rectangle-path-12" data-name="Rectangle-path" class="cls-1" width="14.38" height="2.62" transform="translate(26.063 62.013)"/>
									    <rect id="Rectangle-path-13" data-name="Rectangle-path" class="cls-1" width="18.873" height="2.62" transform="translate(11.684 42.797)"/>
									    <rect id="Rectangle-path-14" data-name="Rectangle-path" class="cls-1" width="33.253" height="2.62" transform="translate(7.19 47.165)"/>
									    <rect id="Rectangle-path-15" data-name="Rectangle-path" class="cls-1" width="7.19" height="2.62" transform="translate(7.19 51.532)"/>
									    <rect id="Rectangle-path-16" data-name="Rectangle-path" class="cls-1" width="23.367" height="2.62" transform="translate(16.177 51.532)"/>
									    <rect id="Rectangle-path-17" data-name="Rectangle-path" class="cls-1" width="7.19" height="2.62" transform="translate(33.253 42.797)"/>
									  </g>
									</svg>
								</div>
								<h4 class="item-title">Content Writhing</h4>
								<p>From Web Design to Web Application Developement we can do whatever you want to fullfull your needs.</p>
							</div>
						</div>
					</div>
				</div>
			</div>
			<?php } ?>
			<!-- Blog section -->
			<?php if( ! empty( $cst_taspristudio['cst_blog_status'] ) ) { ?>
			<div class="blog toggle-section" id="<?php echo esc_html( $blog_menu ); ?>">
				<div class="container">
					<div class="row">
						<div class="col-12">
							<div class="s-title">
								<h2><?php echo $cst_taspristudio['cst_blog_title']; ?></h2>
								<p><?php echo $cst_taspristudio['cst_blog_desc']; ?></p>
							</div>
						</div>
						<?php 

						// Post args

						$args = array(
							'post-type'			=> 'post',
							'orderby'			=> 'ID',
							'post_status'		=> 'publish',
							'order'				=> 'DESC',
							'posts_per_page'		=> 4
						);

						// Query all post by $args
						$result = new WP_Query( $args );
						if ( $result->have_posts() ):
							while ( $result->have_posts() ) : $result->the_post();
						?>
						<div class="col-lg-3 col-sm-6">
							<div class="blog-post">
								<div class="post-thumb">
									<?php the_post_thumbnail( 'medium' ); ?>
									<div class="overlay-btn">
										<a href="<?php the_permalink(); ?>"><?php _e( 'Read More', 'cs_taspristudio' ); ?></a>
									</div>
								</div>
								<div class="post-des">
									<div class="post-meta">
										<span class="meta-category"><a href="#">Mobile Apps</a></span>
										<span class="meta-like"><a href="#">56</a></span>
									</div>
									<h2 class="post-title"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h2>
								</div>
							</div>
						</div>
						<?php endwhile; endif; ?>
					</div>
				</div>		
			</div>
			<?php } ?>
			<!-- Contact section -->
			<?php if( ! empty( $cst_taspristudio['cst_contact_status'] ) ) { ?>
			<div class="contact toggle-section" id="<?php echo esc_html( $contact_menu ); ?>">
				<div class="container">
					<div class="row">
						<div class="col-12">
							<div class="s-title">
								<h2><?php echo $cst_taspristudio['cst_contact_title']; ?></h2>
								<p>Fill out the form and we’ll be in touch as soo as possible to help you out with your problem with the best solution internet can provide so get ready to blown away.</p>
							</div>
						</div>
						<div class="col-lg-6">
							<form class="contact-form">
								<div class="form-group">
									<input type="text" class="form-control" placeholder="Name*">
								</div>
								<div class="form-group">
									<input type="email" class="form-control" placeholder="E-mail*">
								</div>
								<div class="form-group">
									<input type="text" class="form-control" placeholder="Subject">
								</div>
								<div class="form-group">
									<textarea name=""  rows="1" placeholder="Your Message*" class="form-control"></textarea>
								</div>
								<div class="form-submit">
									<button class="f-s-btn btn" type="button">Send</button>
								</div>
							</form>
						</div>
					</div>
				</div>
			</div>
			<?php }?>
		</div>

		<footer class="footer fixed-bottom">
			<div class="container">
				<div class="row no-gutters">
					<?php 
						if ( ! empty( $cst_taspristudio['cst_copyright_text_left'] ) ) { ?>
							<div class="f-col">
								<p class="copyright">
									<?php echo esc_html( $cst_taspristudio['cst_copyright_text_left'] ); ?>
								</p>
							</div>
					<?php } ?>

					<?php 
						if ( ! empty( $cst_taspristudio['cst_copyright_text_center'] ) ) { ?>
							<div class="f-col">
								<p class="copyright">
									<?php echo $cst_taspristudio['cst_copyright_text_center']; ?>
								</p>
							</div>
					<?php }?>

					<?php 
						// Check empty value
						if ( ! empty( $cst_taspristudio['cst_facebook'] ) || ! empty( $cst_taspristudio['cst_twitter'] ) || ! empty( $cst_taspristudio['cst_linkedin'] ) || ! empty( $cst_taspristudio['cst_tumblr'] ) || ! empty( $cst_taspristudio['cst_instagram'] ) || ! empty( $cst_taspristudio['cst_reddit'] ) || ! empty( $cst_taspristudio['cst_youtube'] ) ) { ?>
							<div class="f-col ml-auto">
								<ul class="social-profile">

									<!-- Facebook -->
									<?php if( ! empty( $cst_taspristudio['cst_facebook'] ) ) { ?>
										<li>
											<a href="<?php echo esc_url( $cst_taspristudio['cst_facebook'] ); ?>">
												<i class="fa fa-facebook"></i>
											</a>
										</li>
									<?php } ?>

									<!-- Twitter -->
									<?php if( ! empty( $cst_taspristudio['cst_twitter'] ) ) { ?>
										<li>
											<a href="<?php echo esc_url( $cst_taspristudio['cst_twitter'] ); ?>">
												<i class="fa fa-twitter"></i>
											</a>
										</li>
									<?php } ?>

									<!-- Linkedin -->
									<?php if( ! empty( $cst_taspristudio['cst_linkedin'] ) ) { ?>
										<li>
											<a href="<?php echo esc_url( $cst_taspristudio['cst_linkedin'] ); ?>">
												<i class="fa fa-linkedin"></i>
											</a>
										</li>
									<?php } ?>

									<!-- Tumblr -->
									<?php if( ! empty( $cst_taspristudio['cst_tumblr'] ) ) { ?>
										<li>
											<a href="<?php echo esc_url( $cst_taspristudio['cst_tumblr'] ); ?>">
												<i class="fa fa-tumblr"></i>
											</a>
										</li>
									<?php } ?>

									<!-- Instagram -->
									<?php if( ! empty( $cst_taspristudio['cst_instagram'] ) ) { ?>
										<li>
											<a href="<?php echo esc_url( $cst_taspristudio['cst_instagram'] ); ?>">
												<i class="fa fa-instagram"></i>
											</a>
										</li>
									<?php } ?>

									<!-- Reddit -->
									<?php if( ! empty( $cst_taspristudio['cst_reddit'] ) ) { ?>
										<li>
											<a href="<?php echo esc_url( $cst_taspristudio['cst_reddit'] ); ?>">
												<i class="fa fa-reddit"></i>
											</a>
										</li>
									<?php } ?>

									<!-- Youtube -->
									<?php if( ! empty( $cst_taspristudio['cst_youtube'] ) ) { ?>
										<li>
											<a href="<?php echo esc_url( $cst_taspristudio['cst_youtube'] ); ?>">
												<i class="fa fa-youtube-play"></i>
											</a>
										</li>
									<?php } ?>

								</ul>
							</div>
					<?php } ?>
				</div>
			</div>
		</footer>
		<div data-relative-input="true" id="scene" class="px-wrapper">
			<div class="layer d-none" data-depth="0.2"></div>
		  	<div class="px-layer" data-depth="0.2">
		  		<div class="circle-1"></div>
		  	</div>
		  	<div class="px-layer" data-depth="0.2" data-limit-x="0">
		  		<div class="circle-2"></div>
		  	</div>
		</div>
		<div data-relative-input="true" id="circle"  class="px-wrapper">
			<div class="layer d-none" data-depth="1"></div>
		  	<div class="px-layer" data-depth="1">
		  		<div class="circle-o-1"></div>
		  	</div>
		  	<div class="px-layer" data-depth="1" data-limit-x="0">
		  		<div class="circle-o-2"></div>
		  	</div>
		  	<div class="px-layer" data-depth="1" data-limit-x="0">
		  		<div class="circle-o-3"></div>
		  	</div>
		</div>
		<div data-relative-input="true" id="line"  class="px-wrapper">
			<div class="layer d-none" data-depth="0.4"></div>
		  	<div class="px-layer" data-depth="0.4">
		  		<div class="line-1"></div>
		  	</div>
		  	<div class="px-layer" data-depth="0.4" data-limit-x="0">
		  		<div class="line-2"></div>
		  	</div>
		  	<div class="px-layer" data-depth="0.4" data-limit-x="0">
		  		<div class="line-3"></div>
		  	</div>
		  	<div class="px-layer" data-depth="0.4" data-limit-x="0">
		  		<div class="line-4"></div>
		  	</div>
		</div>
		<div data-relative-input="true" id="star"  class="px-wrapper">
			<div class="layer d-none" data-depth="0.7"></div>
		  	<div class="px-layer" data-depth="0.7">
		  		<div class="star-1"></div>
		  	</div>
		  	<div class="px-layer" data-depth="0.7" data-limit-x="0">
		  		<div class="star-2"></div>
		  	</div>
		  	<div class="px-layer" data-depth="0.7" data-limit-x="0">
		  		<div class="star-3"></div>
		  	</div>
		</div>
		<div data-relative-input="true" id="triangle"  class="px-wrapper">
			<div class="layer d-none" data-depth="0.3"></div>
		  	<div class="px-layer" data-depth="0.3">
		  		<div class="triangle-1"></div>
		  	</div>
		  	<div class="px-layer" data-depth="0.3" data-limit-x="0">
		  		<div class="triangle-2"></div>
		  	</div>
		  	<div class="px-layer" data-depth="0.3" data-limit-x="0">
		  		<div class="triangle-3"></div>
		  	</div>
		  	<div class="px-layer" data-depth="0.3" data-limit-x="0">
		  		<div class="triangle-4"></div>
		  	</div>
		</div>
		<div data-relative-input="true" id="polygon"  class="px-wrapper">
			<div class="layer d-none" data-depth="0.5"></div>
		  	<div class="px-layer" data-depth="0.5">
		  		<div class="polygon-1"></div>
		  	</div>
		  	<div class="px-layer" data-depth="0.5" data-limit-x="0">
		  		<div class="polygon-2"></div>
		  	</div>
		  	<div class="px-layer" data-depth="0.5" data-limit-x="0">
		  		<div class="polygon-3"></div>
		  	</div>
		</div>
	</div>

	<script src="<?php echo CST_ASSETS; ?>/js/jquery-2.2.3.min.js"></script>
	<script src="<?php echo CST_ASSETS; ?>/js/popper.min.js"></script>
	<script src="<?php echo CST_ASSETS; ?>/js/bootstrap.min.js"></script>
	<script src="<?php echo CST_ASSETS; ?>/js/parallax.js"></script>
	<script src="<?php echo CST_ASSETS; ?>/js/jquery.countdown.min.js"></script>
	<script src="<?php echo CST_ASSETS; ?>/js/custom.js"></script>
</body>
</html>